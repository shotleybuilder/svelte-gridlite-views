...
export function rowToView(row: ViewRow): SavedView {
  return {
    ...
    config: {
      ...
      sorting:
        typeof row.sorting === "string" ? JSON.parse(row.sorting) : row.sorting,
      grouping:
        typeof row.grouping === "string"
          ? JSON.parse(row.grouping)
          : row.grouping,
      columnVisibility:
        typeof row.column_visibility === "string"
          ? JSON.parse(row.column_visibility)
          : row.column_visibility,
      columnOrder:
        typeof row.column_order === "string"
          ? JSON.parse(row.column_order)
          : row.column_order,
      columnWidths:
        typeof row.column_widths === "string"
          ? JSON.parse(row.column_widths)
          : row.column_widths,
