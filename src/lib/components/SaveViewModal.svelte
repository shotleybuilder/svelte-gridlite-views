<script lang="ts">
	...
	import type { ViewStoreBundle, ViewConfig } from '../types.js'
	...
	$: hasSorting = config.sorting.length > 0
	$: columnCount = Object.values(config.columnVisibility).filter(Boolean).length || config.columnOrder.length
	...
	async function handleSave() {
		...
		const nameExists = await actions.nameExists(name.trim())
		if (nameExists) {
			error = `View "${name.trim()}" already exists. Please choose a different name.`
			return
		}

		const stats = await actions.getStorageStats()
		...
		try {
			const savedView = await actions.save({
				name: name.trim(),
	...
	>
		...
		>
			...
			<div class="px-6 py-4 border-b border-gray-200">
				<h2 id="modal-title" class="text-xl font-semibold text-gray-900">Save View</h2>
			</div>
					...
					<ul class="text-sm text-gray-600 space-y-1">
						...
						{#if hasSorting}
							<li class="flex items-center gap-2">
								...
								{config.sorting.length} sort {config.sorting.length === 1 ? 'rule' : 'rules'}
							</li>
						{/if}
						{#if columnCount > 0}
							<li class="flex items-center gap-2">
								...
								{columnCount} visible {columnCount === 1 ? 'column' : 'columns'}
							</li>
						{/if}
						<li class="flex items-center gap-2">
							<svg class="w-4 h-4 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
			...
			<div class="px-6 pb-3">
				<p class="text-xs text-gray-500">
					Tip: Press <kbd class="px-1 py-0.5 bg-gray-100 border border-gray-300 rounded text-xs">Esc</kbd> to cancel,
